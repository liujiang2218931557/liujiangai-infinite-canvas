...
+ [调整] 内置六酱 New API 渠道统一改用 `https://liujiangai.cn/v1`，用户在 New API 自行注册、充值并创建 API Key 后粘贴到画布使用。
+ [新增] 提示词来源新增 Freestylefly GPT Image 2 内置来源。
+ [新增] 配置中心预置 AICopy 图片与视频渠道，画布和工作台可选择文档列出的模型并按分支协议请求。
+ [调整] AICopy 需要 HTTPS 素材的模型改为调用用户配置的私有 COS 上传网关，不再默认上传到海外临时托管服务；签名链接在有效期内可复用。
