...
import { Cloud, Download, ExternalLink, KeyRound, Pencil, Plus, RefreshCw, Trash2, Unplug, Upload, Wifi } from "lucide-react";
...
import { AICOPY_CHANNEL_ID } from "@/services/api/aicopy";
...
export function AppConfigPanel({ showDoneButton = false, initialTab = "channels" }: { showDoneButton?: boolean; initialTab?: ConfigTabKey }) {
    ...
    const disconnectNewApi = () => {
        updateChannels(config.channels.map((channel) => channel.id === AICOPY_CHANNEL_ID ? { ...channel, apiKey: "" } : channel));
        message.success("已从这台设备清除画布 API Key；New API 后台的 Key 不会被删除");
    };
    ...
    return (
        <>
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stone-200 pb-3 dark:border-stone-800">
                <div className="text-xs text-stone-500">配置导出不会包含 API Key、上传令牌或 WebDAV 密码。</div>
                                        ...
                                        <div key={channel.id} className="flex items-center justify-between gap-3 rounded-lg border border-stone-200 px-4 py-3 dark:border-stone-800">
                                            ...
                                            <div className="flex shrink-0 gap-2">
                                                {channel.id === AICOPY_CHANNEL_ID ? (
                                                    <>
                                                        <Button size="small" type="primary" icon={<KeyRound className="size-3.5" />} onClick={() => setEditingChannelId(channel.id)}>
                                                            {channel.apiKey ? "管理 Key" : "填写 Key"}
                                                        </Button>
                                                        <Button size="small" icon={<ExternalLink className="size-3.5" />} href="https://liujiangai.cn" target="_blank">
                                                            New API
                                                        </Button>
                                                        {channel.apiKey ? <Button size="small" icon={<Unplug className="size-3.5" />} onClick={disconnectNewApi}>断开</Button> : null}
                                                    </>
                                                ) : null}
