...
const publicMediaCache = new Map<string, string>();
...
function arrayBufferToBase64(value: ArrayBuffer) {
    ...
    for (let offset = 0; offset < bytes.length; offset += 0x8000) batches.push(String.fromCharCode(...bytes.subarray(offset, offset + 0x8000)));
    return btoa(batches.join(""));
}

async function callMediaGateway(endpoint: string, token: string, body: Record<string, unknown>, signal?: AbortSignal) {
    ...
    const result = (await response.json().catch(() => null)) as GatewayResponse | null;
    if (!response.ok || !result) throw new Error(result?.error || `公网素材网关请求失败（${response.status}）`);
    return result;
}

async function uploadPublicMedia(value: string, config: AiConfig, signal?: AbortSignal) {
    if (/^https:\/\//i.test(value)) return value;
    const endpoint = config.mediaUploadUrl.trim();
    if (!endpoint) throw new Error("此模型需要公网素材。请在 AICopy 渠道中配置你自己的 OSS 上传网关。");
    const cacheKey = `${endpoint}:${value}`;
    const cached = publicMediaCache.get(cacheKey);
    if (cached) return cached;
    const response = await fetch(value, { signal });
    if (!response.ok) throw new Error(`无法读取待上传素材（${response.status}）`);
    const blob = await response.blob();
    const token = config.mediaUploadToken.trim();
    const initialized = await callMediaGateway(endpoint, token, { action: "init", filename: "reference", contentType: blob.type || "application/octet-stream", size: blob.size }, signal);
    if (!initialized.session || !Number.isInteger(initialized.chunkSize) || Number(initialized.chunkSize) <= 0) throw new Error("公网素材网关未返回有效的上传会话");
    const session = initialized.session;
    const size = Number(initialized.chunkSize);
    const parts: Array<{ partNumber: number; etag: string }> = [];
    try {
        for (let offset = 0, partNumber = 1; offset < blob.size; offset += size, partNumber += 1) {
            const data = arrayBufferToBase64(await blob.slice(offset, Math.min(offset + size, blob.size)).arrayBuffer());
            const uploaded = await callMediaGateway(endpoint, token, { action: "part", session, partNumber, data }, signal);
            if (!uploaded.etag) throw new Error(`公网素材第 ${partNumber} 块上传失败`);
            parts.push({ partNumber, etag: uploaded.etag });
        }
        const result = await callMediaGateway(endpoint, token, { action: "complete", session, parts }, signal);
        const url = typeof result.url === "string" ? result.url : typeof result.data?.url === "string" ? result.data.url : "";
        if (!/^https:\/\//i.test(url)) throw new Error("公网素材网关未返回 HTTPS 地址");
        publicMediaCache.set(cacheKey, url);
        return url;
    } catch (error) {
        await callMediaGateway(endpoint, token, { action: "abort", session }, undefined).catch(() => undefined);
        throw error;
    }
}
 ...
 *   http / request / poll / sleep / uploadPublicMedia / signal / onDelta    — request helpers
...
export async function runModelPlugin<T = unknown>(args: RunPluginArgs): Promise<T> {
    ...
    try {
        return await runner(
            ...
            (value: string) => uploadPublicMedia(value, config, args.signal),
            args.signal,
            args.onDelta,
        );
    } catch (error) {
...
export function getPluginVariables(): PluginVariable[] {
    return [
        ...
        { name: "uploadPublicMedia", type: "(dataUrl) => Promise<string>", desc: "上传至渠道配置的私有公网素材网关，并返回临时签名 URL；仅用于必须公网素材的模型" },
