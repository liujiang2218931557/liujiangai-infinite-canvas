...
async function resolvePluginMediaUrl(media: ReferenceVideo | ReferenceAudio) {
    if (media.url && (isPublicMediaUrl(media.url) || media.url.startsWith("data:"))) return media.url;
    const blob = media.storageKey ? await getMediaBlob(media.storageKey) : media.url?.startsWith("blob:") ? await (await fetch(media.url)).blob() : null;
