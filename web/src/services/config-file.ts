...
import { enforceBuiltInNewApiChannel, stripExportSecretsFromConfig, stripExportSecretsFromWebdav, useConfigStore, type AiConfig, type WebdavSyncConfig } from "@/stores/use-config-store";
...
export function exportAppConfig() {
    ...
    const data: AppConfigFile = { app: "infinite-canvas", version: 1, exportedAt: new Date().toISOString(), config: stripExportSecretsFromConfig(config), webdav: stripExportSecretsFromWebdav(webdav), promptSources: { sources, schedule } };
...
export async function importAppConfig(file: File) {
    ...
    useConfigStore.setState({ config: enforceBuiltInNewApiChannel(stripExportSecretsFromConfig(data.config)), webdav: stripExportSecretsFromWebdav(data.webdav) });
