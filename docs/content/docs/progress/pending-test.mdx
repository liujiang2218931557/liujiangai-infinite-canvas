...
- Seedance 2.5 Stable Channel: after the matching New API JuKe channel is
  deployed and an ordinary user API key can see `seedance-2.5-stable` through
  `GET /v1/models`, verify that video settings only offer the documented
  duration set (4, 5, 6, 8, 10, 12, 15, 20, 25, 29, 30), 480p/720p, and the
  six supported aspect ratios. Test real generation only with explicit owner
  authorization and a budget limit: task creation must return a New API ID,
  polling must reach a terminal state, and successful video must write back to
  a playable canvas node. Do not put any user token, upstream key, or media
  gateway token in test notes, screenshots, Git, or source files.

- 六酱 New API 渠道：用户应先在 `https://liujiangai.cn` 注册/登录、充值并自行创建普通 API Key；在画布设置中点击「填写 Key」后，Base URL 应固定为 `https://liujiangai.cn/v1`。随后验证图片、视频工作台和画布节点可选择预置模型、`/v1/models` 可正常跨域加载，以及文生图、参考图生图、文生视频、图生视频和带视频/音频参考的多参考视频请求。不要在验收中实际发送会收费的生成请求。要求公网 HTTPS 素材的模型会将本地素材上传至私有 COS 并返回有效期内可复用的签名链接；需确认上传、复用链接、生命周期删除和错误提示正确，且不要以私密素材测试。
